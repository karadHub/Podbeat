...
export const useCommandPalette = ({
  ...
  useEffect(() => {
    const saved = localStorage.getItem('podbeat_search_history');
    if (saved) {
      try {
  ...
  const handleItemSelect = (item: SearchResult) => {
    ...
    if (query.trim()) {
      ...
      localStorage.setItem('podbeat_search_history', JSON.stringify({
        queries: newSearches,
        items: newHistory
      }));
    }
  ...
  return {
    ...
    clearHistory: () => {
      setRecentSearches([]);
      setSearchHistory([]);
      localStorage.removeItem('podbeat_search_history');
    }
  };
};
